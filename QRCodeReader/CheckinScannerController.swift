...
class CheckinScannerController: UIViewController, AVCaptureMetadataOutputObjectsDelegate {
    ...
    @IBAction func manulInput(_ sender: Any) {
        ...
        inputAlert.addAction(UIAlertAction(title: "OK", style: .default) { _ in
            ...
            if (urlInput?.range(of: "^[A-Z]{8}$", options: .regularExpression) != nil) {
                let sureAlert = UIAlertController(title: "Check-in", message: "确定check-in这张门票？", preferredStyle: .alert)
                sureAlert.addAction(UIAlertAction(title: "Yes", style: .default) { _ in
                    let loadingAlert = UIAlertController(title: nil, message: "Check-in中，请稍等...", preferredStyle: .alert)
                    ...
                    let url = "https://vote.cssauw.org/scanapp/mark_used"
                        ...
                        HTTP.GET(url, parameters: params) { response in
                            ...
                            loadingAlert.dismiss(animated: true) {
                                ...
                                if (json != nil) {
                                    ...
                                    if (success) {
                                        let successAlert = UIAlertController(title: "成功Check-in", message: "成功Check-in这张门票！", preferredStyle: .alert)
                                        successAlert.addAction(UIAlertAction(title: "OK", style: .default)  { _ in
                                            self.resetScan()
                                        })
                                        self.present(successAlert, animated: true, completion: nil)
                                    } else {
                                        let failureAlert = UIAlertController(title: "Check-in失败", message: "门票Check-in失败，失败原因：\(message)", preferredStyle: .alert)
